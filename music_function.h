...
void add_music(MYSQL *conn, const char *music_type, const char *music_name)
{
    ...
    char query[256];
    snprintf(query, sizeof(query),
             "INSERT INTO music(`music_type`, `music_name`) VALUES('%s', '%s')",
             music_type, music_name);

    // 执行SQL语句
    if (mysql_query(conn, query))
    {
        fprintf(stderr, "添加音乐失败: %s\n", mysql_error(conn));
        mysql_rollback(conn);      // 回滚事务
        mysql_autocommit(conn, 1); // 恢复自动提交
        return;
    }
...
// 查询music表中所有音乐信息并打印
void query_all_music(MYSQL *conn)
{
    const char *query = "SELECT music_id, music_type, music_name FROM music";
    if (mysql_query(conn, query))
    {
        fprintf(stderr, "查询音乐失败: %s\n", mysql_error(conn));
        return;
    }
    ...
    if (result == NULL)
    {
        fprintf(stderr, "获取音乐结果集失败: %s\n", mysql_error(conn));
        return;
    }

    printf("%-10s %-20s %-30s\n", "music_id", "music_type", "music_name");
    printf("---------------------------------------------------------------\n");
